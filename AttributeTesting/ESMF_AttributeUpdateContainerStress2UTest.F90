...
!   !   Initialization routine.
  ...
  subroutine userm1_init(comp, importState, exportState, clock, rc)
    ...
    field2 = ESMF_FieldCreate(grid, arrayspec=arrayspec, &
    ...
    field3 = ESMF_FieldCreate(grid, arrayspec=arrayspec, &
    ...
    call ESMF_FieldBundleAdd(fieldbundle, (/ field2, field3 /), rc=status)
    if (status .ne. ESMF_SUCCESS) return


    !call ESMF_FieldBundleRemove(fieldbundle, &
    !                            fieldNameList=(/"field2"/), rc=status)
    if (status .ne. ESMF_SUCCESS) return
    ...
    !call ESMF_FieldBundleAddReplace(fieldbundle, fieldList=(/field2/), rc=status)
    if (status .ne. ESMF_SUCCESS) return
...
create fields 2 and 3 and a fieldbundle.  each field receives a standard attribute package
with one of the attributes set.
field2 is removed from fieldbundle and then added back.
...
!   !
  ...
  subroutine userm1_run(comp, importState, exportState, clock, rc)
    ...
    if (rc/=ESMF_SUCCESS) return
    call ESMF_AttributeRemove(field2, name=name, attpack=attpack, rc=rc)
    if (rc/=ESMF_SUCCESS) return

    ! field3: add an attribute to the package
    call ESMF_AttributeGetAttPack(field3, convESMF, purpGen, attpack=attpack, rc=rc)
    if (rc/=ESMF_SUCCESS) return
    call ESMF_AttributeAdd(field3, convESMF, purpGen, attrList=(/"newAtt"/), &
                           attpack=attpack, rc=rc)
    if (rc/=ESMF_SUCCESS) return
    call ESMF_AttributeSet(field3, "newAtt", "funnyValue", attpack=attpack, rc=rc)
    if (rc/=ESMF_SUCCESS) return
#if 0
#endif
...
!-------------------------------------------------------------------------------
    ...
    call ESMF_AttributeGetAttPack(field2, convESMF, purpGen, attpack=attpack, &
                                  rc=rc)
    call ESMF_AttributeGet(field2, attname, value=outVal, attpack=attpack, &
                           isPresent=isPresent, rc=rc)
    write(failMsg, *) "Did not return ESMF_SUCCESS or wrong value"
    write(name, *) "Getting an Attribute package Attribute from field3 test"
    ...
    write(failMsg, *) "Did not return ESMF_SUCCESS or wrong value"
    write(name, *) "Getting an Attribute package Attribute from field3 test"
    call ESMF_Test((rc==ESMF_SUCCESS .and. outVal == "funnyValue"), &
                    name, failMsg, result, ESMF_SRCLINE)
