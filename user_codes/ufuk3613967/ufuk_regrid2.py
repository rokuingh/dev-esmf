...
grid_src = ESMF.Grid(max_index, staggerloc=[ESMF.StaggerLoc.CENTER_VCENTER],
...
max_index = np.array([nx, ny, nz])
grid_dst = ESMF.Grid(max_index, staggerloc=[ESMF.StaggerLoc.CENTER_VCENTER],
                     coord_sys=ESMF.CoordSys.CART)

grid_dst_xc = grid_dst.get_coords(x)
xs = nc.variables['nav_lon'][:]
grid_dst_xc[...] = np.expand_dims(np.swapaxes(xs, 0, 1), 2)

grid_dst_yc = grid_dst.get_coords(y)
ys = nc.variables['nav_lat'][:]
grid_dst_yc[...] = np.expand_dims(np.swapaxes(ys, 0, 1), 2)

grid_dst_zc = grid_dst.get_coords(z)
zs = nc.variables['gdept_0'][0,0:3]
...
# create source field
field_src = ESMF.Field(grid_src, name='field_src', typekind=ESMF.TypeKind.R4,
                       staggerloc=ESMF.StaggerLoc.CENTER_VCENTER)

# create desitination field
field_dst = ESMF.Field(grid_dst, name='field_dst', typekind=ESMF.TypeKind.R4,
                       staggerloc=ESMF.StaggerLoc.CENTER_VCENTER)
...
# get variable
var_src = np.squeeze(nc.variables['temp'][0,0:3,:,:])
