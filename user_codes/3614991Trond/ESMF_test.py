...
def interpolate_from_woa_to_glorys12v1_grids(glorys12v1_ds: xr.Dataset, woa_ds:xr.Dataset):
    ...
    woa_grid, glorys12v1_grid, srcfield, dstfield, xctfield = setup_source_and_destination_3D_grids(woa_ds, glorys12v1_ds)
    # final_array = np.empty((len(woa_ds.time.values), len(glorys12v1_ds.longitude.values),
    #                         len(glorys12v1_ds.latitude.values), len(glorys12v1_ds.depth.values)))
    regrid = setup_ESMF_weights(srcfield, dstfield, mg)
    dstfield.data[:] = 1e20

    woa_ds_transposed = woa_ds.transpose('lon', 'lat', 'depth', 'time')

    print(woa_ds_transposed)
    ...
    for level, lval in enumerate(woa_ds.depth.values):
        for timestep, tval in enumerate(woa_ds.time.values):
            srcfield.data[:,:,level,timestep] = 10.0 + \
              (srclonm*deg2rad)**2 + (srclonm*deg2rad)*(srclatm*deg2rad) + \
              (srclatm*deg2rad)**2
    ...
    # srcfield.data[:,:,:] = woa_ds_transposed["o_an"]
    # dstfield = regrid(srcfield, dstfield)
    # d2 = np.moveaxis(dstfield.data,-1,0)
    # final_array = d2
    # # wouldn't that be nice.. Trond, please send me a formal request for this functionality in ESMPy :)
    # final_array = (dstfield.data).transpose('time', 'lon', 'lat', 'depth',)

    # for t_index, time in enumerate(woa_ds.time):
    #     print("Regridding for time {}".format(t_index))
    #     srcfield.data[:, :, :] = woa_ds_transposed["o_an"][:, :, :, t_index].values
    #     dstfield = regrid(srcfield, dstfield)
    #     final_array[t_index, :, :, :] = dstfield.data
...
def plot_monthly_climatology(climatology):
    ...
    for i in range(12):
        ...
        import ipdb; ipdb.set_trace()
        plt.contourf(climatology.longitude,
                     climatology.latitude,
                     climatology.isel(time=i, depth=0).squeeze(),
                     levels=lev,
                     cmap="RdBu_r",
                     zorder=2,
                     alpha=0.9,
