...
x = np.linspace(start = mesh.coords[0][0].min(), stop = mesh.coords[0][0].max(), num=num)
y = np.linspace(start = mesh.coords[0][1].min(), stop =  mesh.coords[0][1].max(),num=num)
grid = grid_create_from_coordinates(x,y)
dstfield = ESMF.Field(grid,'ID2')

regrid = ESMF.Regrid(srcfield, dstfield, regrid_method=ESMF.RegridMethod.BILINEAR, unmapped_action=ESMF.UnmappedAction.IGNORE)
