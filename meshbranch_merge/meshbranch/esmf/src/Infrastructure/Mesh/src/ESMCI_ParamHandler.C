...
{  
  ...
  {
    ThrowRequire (min_elements <= max_elements);
  }
  ...
  {
    ThrowRequire (seq.find (",") == std::string::npos);
                                 ...
                                 const std::string           &documentation)
{
  ...
  ThrowRequire (pattern.match (default_value));
...
bool ParameterHandler::leave_subsection ()
{
  ...
  ThrowRequire (subsection_path.size() != 0);
...
ParameterHandler::get (const std::string &entry_string) const
{
  ...
  ThrowRequire (pd->entries.find (entry_string) != pd->entries.end());
...
double ParameterHandler::get_double (const std::string &entry_string) const
{
  ...
  double d = std::strtod (s.c_str(), &endptr);
				   // assert there was no error
  ThrowRequire((s.c_str()!='\0') || (*endptr == '\0'));
...
void
ParameterHandler::log_parameters (std::ostream &out)
{
  out << "push:parameters" << std::endl;
...
void
ParameterHandler::log_parameters_section (std::ostream &out)
{
  Section *pd = get_present_defaults_subsection ();
  Section *pc = get_present_changed_subsection ();
