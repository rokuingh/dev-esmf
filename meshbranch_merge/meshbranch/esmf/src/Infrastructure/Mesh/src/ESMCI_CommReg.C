<<<<<<< ESMCI_CommReg.C
=======
// $Id: ESMCI_CommReg.C,v 1.5 2010/03/04 18:57:45 svasquez Exp $
//
...
static const char *const version = "$Id: ESMCI_CommReg.C,v 1.5 2010/03/04 18:57:45 svasquez Exp $";
//-----------------------------------------------------------------------------
  ...
  for (UInt i = 0; i < nfields; i++) {
    if (&sfields[i]->GetMEFamily() != &rfields[i]->GetMEFamily())
      throw Ex() << "Send fields, me for " << sfields[i]->name() << " does not match rfield:"
                 << rfields[i]->name();
    sf.push_back(sfields[i]->Getfield());
    rf.push_back(rfields[i]->Getfield());
  }

  for (UInt j = 0; j < sf.size(); j++)
    obj_type |= sf[j]->UnionType();
...
*/ // don't monitor active
    ...
    //c.clear(Attr::ACTIVE_ID); share this one (12-10-08): update constrained parents.
...
void CommReg::SwapOp(UInt nfields, MEField<> **sfields, int op, const Attr *at) {
  ThrowRequire(dom==ran);
  // Get all the subfields and send out over the specs.
  std::vector<_field*> sf;
  UInt obj_type = 0;
  ...
  if (obj_type & MeshObj::NODE) node_rel.template swap_op<VTYPE,_field>(sf.size(), &sf[0], op, at);
  if (obj_type & MeshObj::EDGE) edge_rel.template swap_op<VTYPE,_field>(sf.size(), &sf[0], op, at);
  if (obj_type & MeshObj::FACE) face_rel.template swap_op<VTYPE,_field>(sf.size(), &sf[0], op, at);
// doesnt make sense for halo  if (obj_type & MeshObj::ELEMENT) elem_rel.halo_fields(sf.size(), &sf[0]);
}
...
template void CommReg::SwapOp<double>(UInt nfields, MEField<> **sfields,int, const Attr*);
template void CommReg::SwapOp<int>(UInt nfields, MEField<> **sfields,int, const Attr*);
template void CommReg::SwapOp<char>(UInt nfields, MEField<> **sfields,int, const Attr*);
template void CommReg::SwapOp<long>(UInt nfields, MEField<> **sfields,int, const Attr*);
