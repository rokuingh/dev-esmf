<<<<<<< ESMCI_CommRel.C
=======
// $Id: ESMCI_CommRel.C,v 1.6 2010/03/04 18:57:45 svasquez Exp $
//
...
static const char *const version = "$Id: ESMCI_CommRel.C,v 1.6 2010/03/04 18:57:45 svasquez Exp $";
//-----------------------------------------------------------------------------
...
>>>>>>> 1.6
  ...
  void CommRel::BuildFromOwner(MeshDB  &dom, const std::vector<CommNode> &obj) 
  {
    ...
    // Wipe out any old state.
    symmetric = true;
    // This creates empty vector and feeds it to 
    // the domain and range vectors
    MapType().swap(domain);
    MapType().swap(range);
    std::vector<UInt>().swap(domain_processors);
    std::vector<UInt>().swap(range_processors);
    ...
    // this is self exchange of procs and object ids
    // We need this before doing the real exchange
    // it inserts our local objected and proc into 
    // the data structures
    ...
    // For each domain processor, get the list of procs that sent to it
    typedef std::map<MeshObj::id_type, std::vector<UInt> > CountMapType;
    CountMapType counts;
    ...
    // Send all the affiliate processors back to the original senders
    SparseMsg msg;

    UInt ndproc = bootStrap.domain_processors.size();
    UInt csize = msg.commSize();
  
    msg.setPattern(ndproc, ndproc == 0 ? NULL : &bootStrap.domain_processors[0]);
    ...
    // Communicate the sizes
    msg.setSizes(ndproc == 0 ? NULL : &sizes[0]);
  ...
  void CommRel::build_range_procs() {
    ...
    // Remove duplicates O(proc*log(proc))
  ...
  void CommRel::complete_range() {
    ...
    SparseMsg msg;
    UInt ndproc = domain_processors.size(); // vector size = number of procs I send to
    UInt csize = msg.commSize(); // Returns comm size = total number of processors in use
    ...
    if (ndproc > 0) {
      ...
      for (; ci != ce; ++ci) {
	UInt proc = ci->processor;
	// ASC: this is enefficient if csize is > ~65k say.
	send_size_all[proc] += SparsePack<MeshObj::id_type>().size(); 
	send_size_all[proc] += SparsePack<UInt>().size();
      }
  
      for (UInt i = 0 ; i < ndproc; i++) {
	sizes[i] = send_size_all[domain_processors[i]]; // reduces vector to ndproc size ...
      }
  ...
  void CommRel::send_fields(UInt _nfields, _field *const *_sfields, _field *const *_rfields) {
    ...
    // Also, the user can send in NULL for rfields, in which case we don't process
    std::set<std::string> pairs_proc;
    UInt nfields = 0;
    std::vector<_field*> sfields, rfields;
    for (UInt f = 0; f < _nfields; f++) {
      if (!_rfields[f]) continue;
...
#ifdef SENDF_DEBUG
	    Par::Out() << "send:" << sfields[f]->name() << " on obj " << MeshObjTypeString(ci->obj->get_type()) << " " << ci->obj->get_id() <<
	      "fdim=" << sfields[f]->dim(obj) << std::endl;
#endif
	    send_size_all[proc] += field_pack_size(*sfields[f])*sfields[f]->dim(obj);
	  } else {
#ifdef SENDF_DEBUG
	    Par::Out() << "send:" << sfields[f]->name() << " not on obj " << MeshObjTypeString(ci->obj->get_type()) << " " << ci->obj->get_id() << std::endl;
#endif
	  }
	} // f
      } // ci
...
>>>>>>> 1.6
...
#ifdef SENDF_DEBUG
	    Par::Out() << "recv:" << rfields[f]->name() << " on obj " << MeshObjTypeString(obj.get_type()) << " " << obj.get_id() <<
	      "fdim=" << rfields[f]->dim(obj) << std::endl;
#endif
	    field_unpack(b, *rfields[f], obj);
	  } else {
#ifdef SENDF_DEBUG
	    Par::Out() << "recv:" << rfields[f]->name() << " not on obj " << MeshObjTypeString(obj.get_type()) << " " << obj.get_id() << std::endl;
#endif
	  }
	}
        ri++;
  ...
  void CommRel::halo_fields(UInt nfields, _field **sfields) const {
	...
	if (GetAttr(*ci->obj).is_locally_owned()) {
	  for (UInt f = 0; f < nfields; f++) {
            if (sfields[f]->OnObj(*ci->obj))
              send_size_all[proc] += SparsePack<double>().size()*sfields[f]->dim(obj);
	  }
	}
      }
