...
static int
wgtFctCmp(float* current, float* nnew, int dim);

/* This function adds the weights */
static int
wgtFctAdd(float* current, float* nnew, int dim);

/* This function chooses the maximum weight */
static int
wgtFctMax(float* current, float* nnew, int dim);
...
wgtFctMax(float* current, float* nnew, int dim)
{
  int i;
  for (i = 0 ; i <dim ; ++i) {
    current[i] = MAX(current[i],nnew[i]);
  }
  return (0);
}
