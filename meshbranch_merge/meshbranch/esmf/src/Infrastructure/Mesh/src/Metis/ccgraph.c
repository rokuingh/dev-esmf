 ...
 * George
 *
 * $Id: ccgraph.c,v 1.1.2.1 2009/02/06 19:16:06 amikstcyr Exp $
 *
 */
...
void ReAdjustMemory(GraphType *graph, GraphType *cgraph, int dovsize) 
{
  ...
  if (cgraph->nedges > 100000 && graph->nedges < 0.7*graph->nedges) {
    ...
    if (graph->ncon == 1) {
      if (dovsize) {
        cgraph->gdata = static_cast<idxtype*>(realloc(cgraph->gdata, (5*cgraph->nvtxs+1 + 2*cgraph->nedges)*sizeof(idxtype)));
        ...
        cgraph->gdata = static_cast<idxtype*>(realloc(cgraph->gdata, (4*cgraph->nvtxs+1 + 2*cgraph->nedges)*sizeof(idxtype)));
      ...
      if (dovsize) {
        ...
        cgraph->gdata = static_cast<idxtype*>(realloc(cgraph->gdata, (3*cgraph->nvtxs+1 + 2*cgraph->nedges)*sizeof(idxtype)));
