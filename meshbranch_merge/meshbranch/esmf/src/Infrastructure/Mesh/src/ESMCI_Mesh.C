<<<<<<< ESMCI_Mesh.C
=======
// $Id: ESMCI_Mesh.C,v 1.5 2010/03/04 18:57:45 svasquez Exp $
//
...
static const char *const version = "$Id: ESMCI_Mesh.C,v 1.5 2010/03/04 18:57:45 svasquez Exp $";
//-----------------------------------------------------------------------------
...
namespace ESMCI {
  ...
  Mesh::Mesh(MeshDB::StoreSizes k) : MeshDB(k), FieldReg(), CommReg(),
				     sghost(NULL),
				     committed(false)
  {
	  ...
	  // Broadcast the object to all sharing processors 
	  {
		...
		if (obj_type == MeshObj::NODE) {
		  ...
		  if (etopo->parametric_dim == 1) {
		    ...
		    MeshObjConn::edge_info(ends.begin(), ends.end(), elems.begin(), elems.end(), 
					   &*ordinals.begin(), &*polaritys.begin());

		  } else {

		    ThrowRequire(etopo->parametric_dim == 2 && spatial_dim() == 3);
		    MeshObjConn::face_info(ends.begin(), ends.end(), elems.begin(), elems.end(), 
		...
		if (obj_type == MeshObj::NODE) {
		  ...
		  MeshObjRelationList::iterator geri = 
		    MeshObjConn::find_relation(**elems.begin(), obj_type, ordinals[0]);
		  ...
		  if (obj_ord < 0) {
...
#ifdef CRE_DEBUG
		      Par::Out() << "Did not find " << MeshObjTypeString(obj_type) 
				 << " on other side, creating" << std::endl; // TODO: take out of sym spec
#endif
...
#ifdef CRE_DEBUG
			Par::Out() << "local obj " << eobj->get_id() 
				   << " not pending create, relinquishing ownership" << std::endl;
#endif
		    ...
		    MeshObjRelationList::iterator cri = 
		      MeshObjConn::find_relation(*geri->obj, obj_type, obj_ord, MeshObj::CHILD);
		...
		if (proc < eobj->get_owner()) {
#ifdef CRE_DEBUG
		  Par::Out() << "Assigning owner " << proc << ", old owner:" << eobj->get_owner() 
			     << ", to obj:" << eobj->get_id() << " -> " << obj_id << std::endl;
#endif
		  ...
		  if (obj_type != MeshObj::NODE && obj_ord < 0) {
			...
			MeshObjConn::find_relation(elem, obj_type, ordinals[i], MeshObj::USES);
		      ...
		      MeshObjRelationList::iterator eri = 
			MeshObjConn::find_relation(*eobj, MeshObj::ELEMENT, ordinals[i], MeshObj::USED_BY);
...
#ifdef CRE_DEBUG
		      Par::Out() << MeshObjTypeString(obj_type) << " " << eobj->get_id() 
				 << " has children.  is_flipped= " << is_flipped << "!!" << std::endl;
#endif
			...
			for (UInt i = 0; i < r0.obj->Relations.size(); i++) {
			  ...
			  if (rel.type == MeshObj::USED_BY) {
			    ...
			    MeshObjRelationList::iterator br = 
			      MeshObjConn::find_relation(*rel.obj, obj_type, rel.ordinal, MeshObj::USES);
	  ...
	  */
	  ...
	  // Keep each message in a local scope, since the SparseMsg size can be quite large.
	  {
...
#ifdef DEL_DEBUG
		Par::Out() << "request about " << obj_id << " from proc:" << proc 
			   << ", del_stat=" << del_stat << std::endl;
#endif
		...
		if (mi == omap.end()) {
		  ...
		  /*
		    if (obj_type != MeshObj::EDGE) {
		    Throw() << "P:" << Par::Rank() << ", Obj not edge!!, " 
		    << MeshObjTypeString(obj_type) << " id=" << obj_id;
		    }
		  */
#ifdef DEL_DEBUG
...
#ifdef DEL_DEBUG
		Par::Out() << "Vote map insert:" << MeshObjTypeString(obj_type) 
			   << " " << obj.get_id() << " from proc:" << proc << ", vote=" << vote << std::endl;
#endif
...
#ifdef DEL_DEBUG
		Par::Out() << "Deleting object: " << MeshObjTypeString(obj_type) << " " 
			   << obj_id << std::endl;
#endif
...
#ifdef DEL_DEBUG
	      Par::Out() << "Deleting object: " 
			 << MeshObjTypeString(obj_type) 
			 << " " << vmi->first << std::endl;
#endif
	  ...
	  for (UInt i = 0; i < to_delete_list.size(); i++) 
	    Par::Out() << "\t" << MeshObjTypeString(to_delete_list[i]->GetType()) 
		       << " " << to_delete_list[i]->get_id() << std::endl;
...
#ifdef DEL_DEBUG
	  {
	    ...
	    for (UInt i = 0; i < to_delete_list.size(); i++)  {
	      CommRel::MapType::iterator lb = std::lower_bound(nrel.domain_begin(), 
							       nrel.domain_end(), 
							       CommRel::CommNode(to_delete_list[i], 0));
	      if (lb != nrel.domain_end() && lb->obj == to_delete_list[i])
		Throw() << "Did not delete object:" 
			<< MeshObjTypeString(to_delete_list[i]->GetType()) 
			<< " " << to_delete_list[i]->get_id();
	    }
	  }
#endif
...
>>>>>>> 1.5
  ...
  void Mesh::build_sym_comm_rel(UInt obj_type) {
	...
	if (oproc != (UInt) Par::Rank()) {
	  //Par::Out() << "proc:" << Par::Rank() << ", shared obj:" 
	  //<< MeshObjTypeString(obj_type) << ", id=" << node.get_id() 
	  //<< ", owner=" << node.get_owner() << std::endl;
