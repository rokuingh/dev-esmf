<<<<<<< ESMCI_Kernel.C
=======
// $Id: ESMCI_Kernel.C,v 1.4 2010/03/04 18:57:45 svasquez Exp $
//
...
static const char *const version = "$Id: ESMCI_Kernel.C,v 1.4 2010/03/04 18:57:45 svasquez Exp $";
//-----------------------------------------------------------------------------
...
void Kernel::Commit(UInt nFields, MEFieldBase **fds, UInt nfields, _field **_fields) {
  ...
  UInt this_NOBJS = mesh.GetStoreSizes().size(type());

  UInt nstore = (objects.size() + this_NOBJS-1) / this_NOBJS;
  ...
  for (UInt i = 0; i < nstore; i++) {
    _fieldStore *s = new _fieldStore(this_NOBJS);
    stores.push_back(*s);
...
//std::cout << "Assimilate pending create obj:" << MeshObjTypeString(obj.get_type()) << " id:" << obj.get_id() << std::endl;
  ...
  // Get a slot in current object.  First object always has space.
 {
    ...
    if (stores.begin() == stores.end()) {
      _fieldStore *ns = new _fieldStore(this_NOBJS);
...
void Kernel::Imprint(MeshObj &obj) const {
  for (UInt i = 0; i < MEFieldme.size(); i++) {
    ...
    if (MEFieldme[mef.GetOrdinal()]) 
      MEImprint(mef.GetCMap(), obj, *MEFieldme[mef.GetOrdinal()]);
  }
}
...
void Kernel::PrintStoreInfo(std::ostream &os) const {
  ...
  for (; si != se; ++si) {
    ...
    //si->PrintMetaData(os, fields.size(), &fields[0]);
...
long Kernel::MemUsage() const {

  long ret = 0;

    // First, remove any empty stores.
  store_const_iterator si = stores.begin(), se = stores.end(), sn;
