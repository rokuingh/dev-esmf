...
public:
  ...
  /**
   * Return how the me permutes dofs that live on edges when
   * edges do not line up.  For instance, lagrange will flip
   * the dofs, whereas hierarchical does not care (sign orientation
   * is taken care of separately).
   */
  virtual const int *edge_perm(bool pol) const = 0;

  /**
   * How to resequence face dofs when faces are not
   * oriented as expected.
   */
  virtual const int *face_perm(UInt rotation, bool pol) const = 0;
...
public:
  ...
  virtual void function_values(UInt npts, UInt fdim, const double pcoord[],
             const field_type fdata[], field_type results[], UInt rstride) const = 0;

  void function_values(UInt npts, UInt fdim, const double pcoord[],
             const field_type fdata[], field_type results[]) const {
    function_values(npts, fdim, pcoord, fdata, results, fdim);
  }

  /** Same as above, but use precomputed shape vals */
  virtual void function_values(UInt npts, UInt fdim, const double pcoord[],
             const field_type fdata[], field_type results[],
             double shape_vals[], UInt rstride) const = 0;

  void function_values(UInt npts, UInt fdim, const double pcoord[],
             const field_type fdata[], field_type results[],
             double shape_vals[]) const {
    function_values(npts, fdim, pcoord, fdata, results, shape_vals, fdim);
  }
  ...
  virtual void function_grads(UInt npts, UInt fdim, const Mapping<typename ME2MPTraits<METRAITS>::value > *mapping,
           ...
           mdata_type shape_grads[], UInt rstride) const = 0;
  ...
  void function_grads(UInt npts, UInt fdim, const Mapping<typename ME2MPTraits<METRAITS>::value > *mapping,
    ...
    function_grads(npts, fdim, mapping, mdata, pcoord, fdata, result, shape_grads, fdim);
  }
...
public:
  ...
  const int *edge_perm(bool pol) const;

  const int *face_perm(UInt rotation, bool pol) const;
...
/**
 * An element field me with no shapefunctions, only storage.
 * @ingroup mesystem
 */
template<class METRAITS = METraits<> >
class MasterElementDG : public MasterElement<METRAITS> {
private:
  MasterElementDG(UInt ndof);
  ~MasterElementDG();
  
  /**
   * Pointer to the shape function object used by this ME.
   */
  static std::map<UInt, MasterElementDG*> meDGMap;
public:
   ...
   * with the same name will return a pre-cached object.
   */ 
  static MasterElementDG *instance(UInt ndof);
  ...
  UInt num_functions() const { return ndof; }

  UInt IntgOrder() const { return 1; }

  // We provide no shape function, grads, etc...
  void shape_function(UInt npts, const double pcoord[], double result[]) const { } 
  ...
  MasterElement<METRAITS> *side_element(UInt side) const { Throw() << "No side element for DG;"; }

  void JxW(const Mapping<typename ME2MPTraits<METRAITS>::value > *mapping, const mdata_type mdata[],
           const intgRule *irule, mdata_type result[]) const {}

  const int *GetDofDescription(UInt dof) const { return &ddesc[4*dof]; }

  UInt NumInterpPoints() const { return 0; }

  void InterpPoints(const MappingBase *mapping,
                const double mdata[], // mapping data
                double result[]) const { }

  const double *InterpPoints() const { return 0; }
  ...
  bool is_nodal() const { return 0; }
  
  UInt orientation() const { return ShapeFunc::ME_DG; }

  const int *edge_perm(bool pol) const { return 0; }

  const int *face_perm(UInt rotation, bool pol) const { return 0; }

  private:

  UInt ndof;
  std::vector<int> ddesc;
