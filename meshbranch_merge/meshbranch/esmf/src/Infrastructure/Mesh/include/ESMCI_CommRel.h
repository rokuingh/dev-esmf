...
namespace ESMCI {
  ...
  public:
    ...
    void swap_op(UInt nfields, FTYPE **sfield, int op, const Attr *at = NULL) const;

    /**
     * Build the communication object for the given swap operation.
     * The penalty of this call is most the MPI_Reducescatter the builds
     * the pattern 
     */
    template <typename VTYPE, typename FTYPE>
    void swap_op_comm(SparseMsg &comm, UInt nfields, FTYPE **sfield, int op, const Attr *at = NULL) const;

    /**
     * Initialize a swap operation.  Isend, Irecv
     */
    template <typename VTYPE, typename FTYPE>
    void swap_op_begin(SparseMsg &msg, UInt nfields, FTYPE **sfield, int op, const Attr *at = NULL) const;

    /**
     * Finalizes a swap operation.  MPI_Waitall
     */
    template <typename VTYPE, typename FTYPE>
    void swap_op_end(SparseMsg &msg, UInt nfields, FTYPE **sfield, int op, const Attr *at = NULL) const;
  ...
  template<typename VTYPE, typename FTYPE>
  void CommRel::swap_op(UInt nfields, FTYPE **sfields, int op, const Attr *at) const {
    if (!symmetric)
      throw Ex() << "swap_op is only implemented for symmetric spec, spec=" << comm_name;
    SparseMsg msg;
    ...
    // Initialize the message send/recv pattern
    if (ndproc > 0) {
    ...
    // Establish the message sizes.  Loop through objects and sum the sizes
    // into each processor we will be sending to so that we have the total
    // message size to each processor.
    MapType::const_iterator ci = domain_begin(), ce = domain_end();
    if (ndproc > 0) {
      std::vector<UInt> send_size_all(csize, 0); 
	...
	for (UInt f = 0; f < nfields; f++) {
	  if (sfields[f]->OnObj(*ci->obj) && (!at || at->subset(GetAttr(*ci->obj))))
	    send_size_all[proc] += SparsePack<VTYPE>().size()*sfields[f]->dim(*ci->obj);
	}
      }
  
      // Set the sizes in a contiguous buffer
      for (UInt i = 0 ; i < ndproc; i++) {
	sizes[i] = send_size_all[domain_processors[i]];
      }
  
      // Communicate the sizes.  Initializes send/recv buffers
      msg.setSizes(&sizes[0]);
    } else
      msg.setSizes(NULL);

    // Pack data into the buffers
    ci = domain_begin();
    for (; ci != ce; ++ci) {
      UInt proc = ci->processor;
    ...
    // Unpack the data.  Do so object by object.
  ...
  /*-------------------------------------------------------------*/
  // Implements an asynchronous swap op.  The SparseComm pattern
  // must be build first, then we can call begin to start the comm,
  // and end to do the MPI_Wait.
  //   1) build comm
  //   2) swap_op_begin
  //   3) swap_op_end
  /*-------------------------------------------------------------*/
  template<typename VTYPE, typename FTYPE>
  void CommRel::swap_op_comm(SparseMsg &comm, UInt nfields, FTYPE **sfields, int op, const Attr *at) const {

    comm.clear(); // reset sparse message.
	...
	sizes[i] = send_size_all[domain_processors[i]];
      ...
      comm.setSizes(&sizes[0]);
    } else
      comm.setSizes(NULL);
  }

  template<typename VTYPE, typename FTYPE>
  void CommRel::swap_op_begin(SparseMsg &msg, UInt nfields, FTYPE **sfields, int op, const Attr *at) const {

    msg.resetBuffers(); // set packing/receiving offsets to beginning
    ...
    // Send messages
    msg.communicate_begin();

  }

  template<typename VTYPE, typename FTYPE>
  void CommRel::swap_op_end(SparseMsg &msg, UInt nfields, FTYPE **sfields, int op, const Attr *at) const {
