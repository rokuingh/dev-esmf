...
template<typename METRAITS,typename FIELD>
void MEValues<METRAITS,FIELD>::Setup(const Kernel &ker, UInt uflag,
      const std::vector<const intgRule*> &irules,
      const Mapping<typename ME2MPTraits<METRAITS>::value> *map) 
{
  ...
  if ((update_flag &MEV::update_sfg) || (update_flag & MEV::update_jxw)) {
    ...
    //update_flag |= MEV::update_jinv;
  }

  // First a quick calculation to figure out sizes
  buf_size += mev_round_to_dword(sizeof(buf_manage));

  if (update_flag & MEV::update_sf) {
    buf_size += mev_round_to_dword(ltotal_nqpoints*lmeptr->num_functions()*sizeof(double));
  }
  if (update_flag & MEV::update_sfg) {
    // param_sfg
    buf_size += mev_round_to_dword(ltotal_nqpoints*lmeptr->num_functions()*topo->parametric_dim*sizeof(double));
    // sfg
    buf_size += mev_round_to_dword(ltotal_nqpoints*sdim*lmeptr->num_functions()*sizeof(double));
  }
  if (update_flag & MEV::update_jxw) {
    buf_size += mev_round_to_dword(ltotal_nqpoints*sizeof(double));
  }
  if (update_flag & MEV::update_map) {
    buf_size += mev_round_to_dword(lcmeptr->num_functions()*sdim*sizeof(double));
  }
  if (update_flag & MEV::update_jinv) {
    buf_size += mev_round_to_dword(ltotal_nqpoints*sdim*sdim*sizeof(double));
  }
  ...
  if (update_flag & MEV::update_jinv) {
    jinv = reinterpret_cast<double*>(&buf[buf_size]);
    buf_size += mev_round_to_dword(ltotal_nqpoints*sdim*sdim*sizeof(double));
  }
...
*/
  ...
  bmg->cur_rule = 0;
  bmg->cur_sf_offset = bmg->cur_sfg_offset = bmg->cur_psfg_offset = bmg->cur_jxw_offset = bmg->cur_jinv_offset = 0;
}
...
void MEValues<METRAITS,FIELD>::ReInit(const MeshObj &elem, UInt rule_num) {
  ...
  if (bmg->num_irules > 0) {
    ...
    for (UInt q = 0; q < rule_num; q++) {
      ...
      bmg->cur_jinv_offset +=  irule_list[q]->npoints()*sdim_sqr;
    }
  ...
  if (update_flag & MEV::update_jinv) {
    for (UInt q = 0; q < bmg->irule->npoints(); q++) 
      mapping->jac_inv(&mdata[0], &bmg->irule->locations()[q*bmg->pdim], &jinv[bmg->cur_jinv_offset+q*sdim_sqr]);
  }
...
template<typename METRAITS,typename FIELD>
void MEValues<METRAITS,FIELD>::GetFunctionValues(const FIELD &field, field_type values[], UInt rstride) {

  if (bmg->meptr->orientation() == ShapeFunc::ME_DG) {

    GetFunctionValues(field, (field_type*)field.data(*cur_elem), values, rstride);
    ...
    GetFunctionValues(field, &fdata[0], values, rstride);
...
void MEValues<METRAITS,FIELD>::GetCoordinateValues(double values[]) {
  ...
  bmg->cmeptr->function_values(bmg->irule->npoints(), cf->dim(), bmg->irule->locations(), &mdata[0], &values[0], cf->dim());
}
...
template<typename METRAITS,typename FIELD>
void MEValues<METRAITS,FIELD>::GetFunctionGrads(const FIELD &field, field_type grads[], UInt rstride) {
  std::vector<field_type> fdata; 

  fdata.resize(bmg->meptr->num_functions()*field.dim());
  GatherElemData<METRAITS,FIELD>(*bmg->meptr, field, *cur_elem, &fdata[0]);

  GetFunctionGrads(field, &fdata[0], grads, rstride);
...
template<typename METRAITS,typename FIELD>
void MESideValues<METRAITS,FIELD>::GetUnitNormals(double unit_normals[]) {
  std::vector<double> lmdata; // different mdata, side data

  lmdata.resize(side_cmeptr->num_functions()*this->sdim);

  GatherSideData<>(*side_cmeptr, *this->cf, *this->cur_elem, cur_side, &lmdata[0]);

  side_mapping->unit_normal(this->bmg->irule->npoints(), &lmdata[0], this->bmg->irule->locations(), &unit_normals[0]);
}

template<typename METRAITS,typename FIELD>
void MESideValues<METRAITS,FIELD>::GetNormals(double normals[]) {
  std::vector<double> lmdata; // different mdata, side data
  ...
  side_mapping->normal(this->bmg->irule->npoints(), &lmdata[0], this->bmg->irule->locations(), &normals[0]);
}
