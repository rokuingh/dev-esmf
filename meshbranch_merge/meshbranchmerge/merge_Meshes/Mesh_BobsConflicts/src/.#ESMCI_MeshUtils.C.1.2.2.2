...
template<typename METRAITS, typename FTYPE, typename RESTYPE, typename DATATYPE>
GatherElemData<METRAITS,FTYPE,RESTYPE,DATATYPE>::GatherElemData(const MasterElement<METRAITS> &me, const FTYPE &f,
     const MeshObj &obj, RESTYPE res[]) 
{
  ...
  if (me.is_nodal()) {
    ...
    bool has_sign = me.orientation() == ShapeFunc::ME_SIGN_ORIENTED ? true : false;

    const int *ddl = bad_dof;
    MeshObjRelationList::const_iterator ri;

    const int *dd; // curent dof descr

    const MeshObj *gobj = 0;
    UInt n = 0; // results buffer counter.

    // Orientation of object.
    bool pol = true;
    int rot = 0;

    // The low level field pointer
    const typename FTYPE::field_type &llf = *f.Getfield();

    const DATATYPE *data = 0; // data

    // Permutation for sides/edges
    const int *perm_table = 0;
   
    // *** Loop dofs ***
    for (UInt i = 0; i < me.num_functions(); i++) {

      // Gather a description of the field.
      UInt nval = me.GetDofValSet(i);
      dd = me.GetDofDescription(i); // handle to dof descr

      // See if we at a --new object--.  This happens either if we move to a new
      // object type, or a new object index.
      if ((dd[1] != ddl[1]) || (dd[0] != ddl[0])) {
        ...
        if (dof2mtype(dd[0]) != MeshObj::ELEMENT) {
          ...
          // Get the appropriate permutation table
          if (dof2mtype(dd[0]) == MeshObj::EDGE) {
            perm_table = me.edge_perm(pol);
          } else if (dof2mtype(dd[0]) == MeshObj::FACE) {
            perm_table = me.face_perm(rot, pol);
          } else perm_table = 0;
        ...
        ddl = dd; // make this object the current one
      ...
      // For nodes,elems or nval == 1 objects, no permutations are needed,
      // so split this case
      if (dd[0] == DOF_NODE || dd[0] == DOF_ELEM || nval == 1 ) {
        ...
        // Object has polarity to take into account

        int idx = perm_table[dd[2]]*fdim;

        // Split here into the two cases of has_sign or not

        if (has_sign && !pol) {

          for (UInt d = 0; d < fdim; d++) {
            res[n++] = dd[3]*data[idx+d];
          }
...
template struct GatherElemData<METraits<>,MEField<>,fad_type,double>;
     ...
     const MeshObj &elem, UInt ordinal, RESTYPE res[]) 
{
  ...
  UInt side_type = GetMeshObjMesh(elem).side_type();
  ...
  if (me.is_nodal()) {
    ...
    MeshObjRelationList::const_iterator cri = 
       MeshObjConn::find_relation(elem, side_type, ordinal);

    ThrowAssert(cri != elem.Relations.end());
    MeshObj &obj = *cri->obj;
    ...
    for (UInt i = 0; i < me.num_functions(); i++) {
      ...
      if ((dd[1] != ddl[1]) || (dd[0] != ddl[0])) {
        if (dof2mtype(dd[0]) != MeshObj::ELEMENT) {
          ...
          // A 3d Face could request edges, but then we must deal with polarity issues,
          // so bomb for the moment in that case.
          const int *onodes = dof2mtype(dd[0]) == MeshObj::NODE ? etopo->get_side_nodes(ordinal) : 0;
          ThrowRequire(onodes);


          ri = MeshObjConn::find_relation(elem, dof2mtype(dd[0]), onodes[dd[1]]);
          if (ri == obj.Relations.end())
            Throw() << "Could not get object:" << MeshObjTypeString(dof2mtype(dd[0])) << ", ord:" << dd[1]
                    << ", obj=" << obj;
          gobj = ri->obj; pol = ri->polarity; rot = ri->rotation;
    ...
    for (UInt i = 0; i < me.num_functions(); i++) {
      ...
      if ((dd[1] != ddl[1]) || (dd[0] != ddl[0])) {
        if (dof2mtype(dd[0]) != MeshObj::ELEMENT) {
          ...
          ri = MeshObjConn::find_relation(obj, dof2mtype(dd[0]), onodes[dd[1]]);
          if (ri == obj.Relations.end())
            Throw() << "Could not get object:" << MeshObjTypeString(dof2mtype(dd[0])) << ", ord:" << dd[1]
                    << ", obj=" << obj;
          gobj = ri->obj; pol = ri->polarity; rot = ri->rotation;
...
/**
 * In this gather we figure the side is oriented correctly wrt us, and so
 * we only must worry about gathering the nodes correctly.
 */
template<typename METRAITS, typename FTYPE, typename RESTYPE>
GatherSideData1<METRAITS,FTYPE,RESTYPE>::GatherSideData1(const MasterElement<METRAITS> &me, const FTYPE &f,
     const MeshObj &elem, UInt ordinal, bool polarity, UChar rotation, RESTYPE res[]) 
{
  UInt fdim = f.dim();
  ...
  //const MeshObjTopo *side_topo = etopo->side_topo(ordinal);
  const int *side_nodes = etopo->get_side_nodes(ordinal);
  const int *perm = etopo->side_topo(ordinal)->perm_table(rotation, polarity);
  ...
  if (me.is_nodal()) {
    ...
    for (UInt n = 0; n < me.num_functions(); n++) {
      ...
      const MeshObj &node = *elem.Relations[side_nodes[perm[n]]].obj;
    ...
    for (UInt i = 0; i < me.num_functions(); i++) {
      ...
      if ((dd[1] != ddl[1]) || (dd[0] != ddl[0])) {
        if (dof2mtype(dd[0]) != MeshObj::ELEMENT) {
          ...
          // A 3d Face could request edges, but then we must deal with polarity issues,
          // so bomb for the moment in that case.
          const int *onodes = dof2mtype(dd[0]) == MeshObj::NODE ? etopo->get_side_nodes(ordinal) :
                     (dof2mtype(dd[0] == MeshObj::EDGE) ? etopo->get_face_edge(ordinal) : 0);
          ThrowRequire(onodes);


          ri = MeshObjConn::find_relation(elem, dof2mtype(dd[0]), onodes[perm[dd[1]]]);
          if (ri == obj.Relations.end())
            Throw() << "Could not get object:" << MeshObjTypeString(dof2mtype(dd[0])) << ", ord:" << dd[1]
                    << ", obj=" << obj;
          gobj = ri->obj; pol = ri->polarity; rot = ri->rotation;
...
template struct GatherSideData1<METraits<>,MEField<>,double>;
template struct GatherSideData1<METraits<>,MEField<>,long>;
template struct GatherSideData1<METraits<>,MEField<>,int>;
template struct GatherSideData1<METraits<fad_type,double>,MEField<SField>,fad_type>;

template<typename METRAITS, typename FTYPE>
void ScatterSideData1(const MasterElement<METRAITS> &me, const FTYPE &f, const MeshObj &elem, UInt ordinal, bool polarity, UChar rotation, const double mcoef[]) 
{
  UInt fdim = f.dim();
  ...
  if (me.is_nodal()) {
    ...
    for (UInt n = 0; n < me.num_functions(); n++) {
      ...
      double *data = f.data(node);

      for (UInt d = 0; d < f.dim(); d++) {
        data[d] = mcoef[n*fdim+d];
      } // dim
...
template void ScatterSideData1(const MasterElement<METraits<> > &me, const MEField<> &f, const MeshObj &elem, UInt ordinal, bool polarity, UChar rotation, const double mcoef[]);
...
double ElemDiameter(Mesh &mesh, MeshObj &elem) {

  MEField<> &c_ref = *mesh.GetCoordField();

  // Loop over parent's nodes
  MeshObjRelationList::iterator n_i  = elem.Relations.begin();

  int num_parent_nodes = 0;

  double max_diam = 0;
  double * coord0 = c_ref.data(*n_i->obj); // child 0
  for ( ; n_i != elem.Relations.end() &&
