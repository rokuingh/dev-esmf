...
static UInt share_parametric_dim(UInt pdim) {
  ...
  void LoadExMesh(Mesh &mesh, const std::string &filename, int nstep) {
	...
	*/
    ...
    std::vector<double> xx(nnodes+1), yy(nnodes+1), zz(nnodes+1);
    for (i=0; i<num_nodes; i++) {
      xx[i] = 0.0;
      yy[i] = 0.0;
      zz[i] = 0.0;
    }
    exoerr = ex_get_coord(exoid,&xx[0],&yy[0],&zz[0]);

    std::vector<int> mm(nnodes+1);
    exoerr = ex_get_node_num_map(exoid, &mm[0]);
    ...
    std::vector<int> blk_id(nblocks+1);
    exoerr = ex_get_elem_blk_ids (exoid, &blk_id[0]);
    int eid = 0;
    ...
    for (m=0, i=0; i<nblocks; i++) {
      ...
      //printf("\tnpe:%d, nel:%d\n", npe, nbelems[i]);
      std::vector<int> c0(npe*nbelems[i]+1);
      exoerr = ex_get_elem_conn(exoid,blk_id[i],&c0[0]);
    ...
    for (int l = 0; l < nss; l++) {
      ...
      // print the side set
      std::vector<int> elist(nsc+1), esides(nsc+1);
  ...
  void WriteExMesh(const Mesh &mesh, const std::string &filename, int nstep, double tstep) {
	...
	if (mf.Output() && (mf().UnionType() & MeshObj::NODE)) {
	...
	if (mf.Output() && (mf().UnionType() & MeshObj::ELEMENT)) {
	  ...
	  if (mf.dim() > 1) {
		...
		// of the same name
	      } else std::sprintf(buf, "_%s_%d", mf.name().c_str(), d); 
	      var_names_ptr.push_back(buf);
