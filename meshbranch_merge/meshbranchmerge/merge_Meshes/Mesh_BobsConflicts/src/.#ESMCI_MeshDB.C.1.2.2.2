...
MeshDB::~MeshDB() {
 ...
 MeshDB::iterator oi = obj_begin_all(), oe = obj_end_all();
//long long num_delete = 0;
...
//if (Par::Rank() == 0) std::cout << "deleted:" << num_delete << " objs" << std::endl;
...
void MeshDB::UseEdges(bool val) {
  ...
  // If in 3d, use edges requires use faces, so:
  use_sides = true;
...
void MeshDB::push_back_sorted(MeshObj &obj, const Attr &attr, const MeshObjTopo *topo) {
  Trace __trace("MeshDB::push_back_sorted(MeshObj &obj, const Attr &attr, const MeshObjTopo *topo)");

  Attr nattr(attr);

  nattr.GetContext().set(Attr::OMEGA_ID);
 
  //int ikey = obj.get_int(key);
  KernelList::iterator lb = std::lower_bound(setRoster.begin(), setRoster.end(), nattr, md_attr_comp());
  Kernel *md;
 
  if (lb == setRoster.end() ||
      nattr != lb->GetAttr()) {
    md = new Kernel(*this, nattr);
    md->topo = topo;
    if (topo) {
...
//std::cout << "New Kernel, attr=" << nattr << std::endl;
  } else md = &(*lb);

  // Objects with the same attr should share the topo.
  if (md->topo != topo) 
    Throw() << "New Kernel, topos don't match:old=" <<
        (md->topo ? md->topo->name : "NULL") << " new=" << 
...
void MeshDB::MemUsage(std::ostream &os) const {

  long obj_use = 0, rel_use = 0;

  // First the usage by objects:
  ...
  for (; oi != oe; ++oi) {
    ...
    obj_use += sizeof(MeshObj);

    rel_use += sizeof(MeshObj::Relation)*obj.Relations.size();

  }

  long field_store = 0;
  
  KernelList::const_iterator ki = set_begin(), ke = set_end(), kn;
  for (; ki != ke; ++ki) {

    field_store += ki->MemUsage();
     
  }

  os << "MeshDB Memusage:" << obj_use + rel_use + field_store << " bytes" << std::endl;
  os << "\tMeshObj use:" << obj_use << " bytes" << std::endl;
  os << "\tMeshObj Relation use:" << rel_use << " bytes" << std::endl;
  os << "\tMeshObj field:" << field_store << " bytes" << std::endl;
