...
/**
 * Register a low level field on a given locale.  The field may already exist, or not,
 * but a pointer to the field is returned in any case.
 */
_field *Registerfield(const std::string &name,
               const _fieldLocale &floc, const _fieldTypeBase &_ftype);

/**
 * Register the field schema, but no locale.
 */
_field *Registerfield(const std::string &name, const _fieldTypeBase &_ftype);
