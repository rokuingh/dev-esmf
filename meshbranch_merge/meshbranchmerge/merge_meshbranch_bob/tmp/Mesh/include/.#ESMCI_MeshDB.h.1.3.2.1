...
/**
 * Struct to describe kernel sizes for each mesh obj type.
 */
struct StoreSizes {
  // Defaults
  StoreSizes() :
    ...
    face_num(100) 
  {}
  UInt elem_num;    // number of elems per kernel
  UInt node_num;    
  UInt edge_num;   
  UInt face_num;  

  UInt size(UInt obj_type) const {
    switch (obj_type) {
      case MeshObj::NODE:
        return node_num;
      case MeshObj::EDGE:
      ...
      case MeshObj::ELEMENT:
        return elem_num;
      default:
        Throw() << "bad object type:" << obj_type << " in StoreSizes.size";
    }
  }
};

MeshDB(const StoreSizes k);
virtual ~MeshDB();
...
/**
 * Return the field store size array
 */
const StoreSizes &GetStoreSizes() { return ksizes; }


/**
 * Print out the memory usage statistics.
 */
void MemUsage(std::ostream&) const;
