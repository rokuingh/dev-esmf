...
void MasterElementV<METRAITS>::function_grads(UInt npts, UInt fdim,
   ...
   mdata_type shape_grads[], UInt rstride) const 
{
  UInt sdim = mapping->spatial_dim();
  UInt ndofs = m_shape->NumFunctions();
 ...
 //   UInt r1 = p*fdim*sdim;
    UInt r1 = p*rstride*sdim;
    UInt sg1 = p*ndofs*sdim;
    ...
    for (UInt fd = 0; fd < fdim; fd++) {
      ...
      for (UInt d = 0; d < sdim; d++) {
        ...
        for (UInt n = 0; n < ndofs; n++) {
          ...
          //result[r1+r2+d] += fdata[n*fdim +fd]*shape_grads[(p*ndofs+n)*sdim+d];
          result[r1+r2+d] += fdata[n*fdim +fd]*shape_grads[sg1+n*sdim+d];
        }
      }
    }
  }
  ...
  mdata_type shape_grads[]) const 
{
  ...
  function_grads(npts, sdim, mapping, mdata, pcoord, fdata, &fgrads[0], shape_grads, sdim);
...
void MasterElementV<METRAITS>::function_values(
               UInt npts, UInt fdim, const double pcoord[],
                const field_type fdata[], field_type results[], UInt rstride) const
{
  UInt ndofs = m_shape->NumFunctions();
  std::vector<double> svals(npts*ndofs);
  m_shape->shape(npts, pcoord, &svals[0]);

  function_values(npts, fdim, pcoord, fdata, results, &svals[0], rstride);
...
void MasterElementV<METRAITS>::function_values(
               ...
               double shape_vals[], UInt rstride) const
{
  UInt ndofs = m_shape->NumFunctions();
  ...
  for (UInt j = 0; j < npts; j++) {
   ...
   for (UInt f = 0; f < fdim; f++) {
     results[r1 + f] = 0;
     for (UInt n = 0; n < ndofs; n++) {
        results[r1 + f] += shape_vals[j*ndofs+n]*fdata[n*fdim + f];
      }
    }
  }
...
template <class METRAITS>
const int *MasterElementV<METRAITS>::edge_perm(bool p) const
{
  return m_shape->edge_perm(p);
}

template <class METRAITS>
const int *MasterElementV<METRAITS>::face_perm(UInt rot, bool p) const
{
  return m_shape->face_perm(rot, p);
}
...
template <class METRAITS>
std::map<UInt, MasterElementDG<METRAITS>*>  MasterElementDG<METRAITS>::meDGMap;

template <class METRAITS>
MasterElementDG<METRAITS> *MasterElementDG<METRAITS>::instance(UInt ndof) {
  typename std::map<UInt, MasterElementDG<METRAITS>*>::iterator mi =
    meDGMap.find(ndof);
  MasterElementDG *me;
  if (mi == meDGMap.end()) {
    me = new MasterElementDG(ndof);
    meDGMap[ndof] = me;
  } else me = mi->second;
...
static std::string get_dg_name(UInt ndof) {
  char buf[512];
...
template <class METRAITS>
MasterElementDG<METRAITS>::MasterElementDG(UInt _ndof) :
ndof(_ndof),
MasterElement<METRAITS>(get_dg_name(_ndof))
{

  for (UInt i = 0; i < ndof; ++i) {
    ddesc.push_back(DOF_ELEM);
    ddesc.push_back(0); // ordinal
    ddesc.push_back(i); // index
    ddesc.push_back(1); // orientation
  }
...
template <class METRAITS>
MasterElementDG<METRAITS>::~MasterElementDG()
{
}

template <class METRAITS>
MasterElement<METraits<> > *MasterElementDG<METRAITS>::operator()(METraits<>) const {
  return MasterElementDG<METraits<> >::instance(ndof);
}

template <class METRAITS>
MasterElement<METraits<fad_type,double> > *MasterElementDG<METRAITS>::operator()(METraits<fad_type,double>) const {
  return MasterElementDG<METraits<fad_type,double> >::instance(ndof);
}

template <class METRAITS>
MasterElement<METraits<double,fad_type> > *MasterElementDG<METRAITS>::operator()(METraits<double,fad_type>) const {
  return MasterElementDG<METraits<double,fad_type> >::instance(ndof);
}

template <class METRAITS>
MasterElement<METraits<fad_type,fad_type> > *MasterElementDG<METRAITS>::operator()(METraits<fad_type,fad_type>) const {
  return MasterElementDG<METraits<fad_type,fad_type> >::instance(ndof);
}

template class MasterElementDG<METraits<> >;
template class MasterElementDG<METraits<fad_type,double> >;
template class MasterElementDG<METraits<double,fad_type> >;
template class MasterElementDG<METraits<fad_type,fad_type> >;
