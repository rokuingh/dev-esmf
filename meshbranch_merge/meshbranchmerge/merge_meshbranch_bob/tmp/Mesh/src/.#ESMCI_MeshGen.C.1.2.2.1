...
void DGMesh(const Mesh &srcmesh, Mesh &dgmesh) {
  ...
  ThrowRequire(srcmesh.is_committed());

  UInt rank = Par::Rank();

  UInt sdim = srcmesh.spatial_dim();
  dgmesh.set_spatial_dimension(srcmesh.spatial_dim());
  dgmesh.set_parametric_dimension(srcmesh.parametric_dim());

  MEField<> &coord = *srcmesh.GetCoordField();


  // Nodes first. Loop elements and create nodes.

  Mesh::const_iterator ei = srcmesh.elem_begin(), ee = srcmesh.elem_end();

  // Count nodes first so we can get unique global ids
  UInt nlocal_nodes = 0;
  for (; ei != ee; ++ei) {
    ...
    nlocal_nodes += etopo->num_nodes;

  } // ei


  // Get global ids.  This is a bit overkill, but will give unique ids
  std::vector<long> used_ids;
  std::vector<long> needed_ids(nlocal_nodes, 0);

  GlobalIds(used_ids, needed_ids);

  // Create the nodes
  nlocal_nodes = 0;
  std::vector<MeshObj*> node_sav(needed_ids.size());
  for (ei = srcmesh.elem_begin(); ei != ee; ++ei) {
    ...
    for (UInt n = 0; n < etopo->num_nodes; ++n) {
      ...
      ThrowAssert(nlocal_nodes < needed_ids.size());
      MeshObj *node = new MeshObj(MeshObj::NODE, needed_ids[nlocal_nodes], nlocal_nodes);

      node->set_owner(rank);

      int nset = 0;
      dgmesh.add_node(node, nset);

      node_sav[nlocal_nodes] = node;
  ...
  for (ei = srcmesh.elem_begin(); ei != ee; ++ei) {
    ...
    MeshObj *nelem = new MeshObj(MeshObj::ELEMENT, elem.get_id(), ++num_elem);

    nelem->set_owner(rank);

    std::vector<MeshObj*> nodes(etopo->num_nodes);

    for (UInt n = 0; n < etopo->num_nodes; ++n) {

      nodes[n] = node_sav[nlocal_nodes++];

    } // n

    int block = 1;
    dgmesh.add_element(nelem, nodes, block, etopo);

  } // ei

  // Set up the coordinates to match the unit cube.
  IOField<NodalField> *node_coord = dgmesh.RegisterNodalField(dgmesh, "coordinates", sdim);
  ...
  for (ei = srcmesh.elem_begin(); ei != ee; ++ei) {
    ...
    for (UInt n = 0; n < etopo->num_nodes; ++n) {
      ...
      const MeshObj &nnode = *node_sav[nlocal_nodes++];

      MeshObj &onode = *elem.Relations[n].obj;

      double *oc = coord.data(onode);
      double *nc = node_coord->data(nnode);

      std::copy(oc, oc + sdim, nc);
