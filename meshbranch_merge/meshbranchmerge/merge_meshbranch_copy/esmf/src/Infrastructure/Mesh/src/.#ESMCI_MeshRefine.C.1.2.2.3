...
/**
 * Connect an edge to any children elements of a refining element
 */
void connect_edge_to_children(const RefineTopo &rtopo,         // Refinement topo of element
                              MeshObj &edge,                   // The edge to add
                              const MeshObjTopo *pfctopo,      // topo of edge
                              std::vector<MeshObj*> &pfcnodes, // Edges nodes
                              std::vector<MeshObj*> &children) // Child elements
{
...
*/
    ...
    if (ordinal >= 0) { // found it
      ...
      AddMeshObjRelation(edge, r); // add child to edge
      found_child = true;
    }
  } // children
  if (!found_child) Throw() << "Edge " << edge << "did not find host child element!";
}

/** True if the face is genesis or has a genesis parent face (non interior). */
bool has_genesis_parent_face(MeshObj &obj) {

  ThrowAssert(obj.get_type() == MeshObj::FACE);


  // Look upward.
  MeshObjRelationList::iterator pi =
      MeshObjConn::find_relation(obj, MeshObj::FACE);

  bool parent_found = false;

  for (; pi != obj.Relations.end() && pi->obj->get_type() == MeshObj::FACE &&
           !(parent_found = (pi->type == MeshObj::PARENT)); ++pi);

  if (parent_found) return has_genesis_parent_face(*pi->obj);

  const Context &ctxt = GetMeshObjContext(obj);

  return ctxt.is_set(Attr::GENESIS_ID);

}

/**
 * Connect the (new) child faces up to the child elements.
 * Also, add any nodes that a face hosts that may have just been created to
 * elements around that may parent them.
 * And, lastly, if UseEdges, and if obj has a genesis face parent (meaning
 * it could end up on a processor boundary), see if we have FOSTERS edges;
 * if so, add them to the new child elements.  If not, create them and do
 * so.
 */
void face_resolution(MeshDB &mesh,
                     MeshObj &obj,
...
Par::Out() << "should add to? " << nobj.get_id() << std::endl;
    ...
    // This is a convenient (if obscure) place to deal with fostered edges.
    // we may be able to provide certain edges to the children elements.
    // Note that only the genesis outer faces will have these, so don't worry
    // if they are not there.
    // Only applies to faces that live on a parent genesis face and could become
    // exposed on a parallel boundary.
    if (0 && mesh.AllEdges() && has_genesis_parent_face(*pfi->obj)) {
      ThrowRequire(has_face);

      MeshObj &face = *pfi->obj;

      const MeshObjTopo *edge_topo = pftopo->side_topo(0); // edges are homogeneous

      for (UInt fe = 0; fe < pftopo->num_fostered_edges; ++fe) {

        MeshObjRelationList::iterator fi =
          MeshObjConn::find_relation(face, MeshObj::EDGE, fe, MeshObj::FOSTERS);
  
        bool fe_polarity=true;
        MeshObj *fedge = 0;
  
        if (fi == face.Relations.end()) { 
          // Must create the edge.
          Context newctxt;
          newctxt.set(Attr::PENDING_CREATE_ID);
          newctxt.set(Attr::ACTIVE_ID);
          Attr na(MeshObj::EDGE, newctxt);

          fedge = new MeshObj(MeshObj::EDGE, mesh.get_new_local_id(MeshObj::EDGE));

          MeshObj::Relation r;
          r.obj = fedge;
          r.ordinal = fe;
          r.type = MeshObj::FOSTERS;
          AddMeshObjRelation(face, r); // add edge to child
          r.obj = &face;
          r.type = MeshObj::FOSTERED_BY;
          AddMeshObjRelation(*fedge, r);

          mesh.add_object(fedge, na, edge_topo);
Par::Out() << "Creating fostered edge " << fe << std::endl;
  
        } else {
          fe_polarity = fi->polarity;
          fedge = fi->obj;
Par::Out() << "Found fostered edge " << fe << std::endl;
        }

        // Acquire the nodes for the edge; connect edge to the children elements
        const int *edge_perm = edge_topo->perm_table(0, fe_polarity ? 1 : 0);
  
        // Lets get the nodes of the edge.
        std::vector<MeshObj*> enodes(edge_topo->num_nodes);
  
        const int *fostered_edge_nodes =
           pftopo->get_fostered_edge_nodes(fe);
  
        for (UInt i = 0; i < edge_topo->num_nodes; ++i) {
  
          // Permutation table mania!!!
          UInt element_node_idx = side_nodes[perm[fostered_edge_nodes[edge_perm[i]]]];
            
          enodes[i] = obj.Relations[element_node_idx].obj;
  
        } // i

        // Connects edge to child elements.  It finally has a real home, not just a
        // foster home!!
        connect_edge_to_children(rtopo, 
                              *fedge,
                              edge_topo,
                              enodes,
                              children);
...
/**
 * Add the new child edges to the child elements.  Also, add any child
 * nodes of the edges to elements surrounding.
 */
void edge_resolution(MeshDB &mesh,
                     MeshObj &obj,
                     const RefineTopo &rtopo,
                     std::vector<MeshObj*> &children,
                     std::vector<MeshObj*> &nodes)
{
  ...
  for (UInt pf = 0; pf < (UInt) topo->num_edges; pf++) {
    ...
    /*------------------------------------------------------------*/
    // Loop child edges
    if (has_edge) // 12-22-07 (It doesn't appear that we need to do this unless we have the edge)
    for (UInt pfc = 0; pfc < pfrtopo->NumChild(); pfc++) {
      MeshObjRelationList::iterator pfci;
      if (has_edge) {
...
//Par::Out() << " found: " << pfcni->obj->get_id() << std::endl;
      ...
      /*----------------------------------------------------------*/
      // Goal 1) Add edge to the children elements
      if (has_edge) {
        connect_edge_to_children(rtopo,  
                                 *pfci->obj,
                                 pfctopo, 
    ...
    /*----------------------------------------------------------*/
    // Goal 2) For the parent edge, find all elements around that
    //   use the edge (virtual or not) and see if we can provide any
    //   child nodes of the edge to the elements around.

    // Gather the parent objects nodes.
    std::vector<MeshObj*> pfnodes(pftopo->num_nodes);
    for (UInt i = 0; i < pftopo->num_nodes; i++) {
...
dPar::Out() << "n1:" << pfnodes[0]->get_id() << ", " << pfnodes[1]->get_id() << std::endl;
    ...
    // Get neighbors of the edge (using nodes)
    std::vector<MeshObj*> neighbors;
    MeshObjConn::common_objs(pfnodes.begin(), pfnodes.end(), MeshObj::USED_BY, MeshObj::ELEMENT, neighbors);

    if (neighbors.size() > 0) {

      // Found some, so see if how this edge relates to them
      std::vector<int> ordinals(neighbors.size());
      std::vector<int> polaritys(neighbors.size());
...
Par::Out() << "edge:should add to? " << nobj.get_id() << " ";
        ...
        // Loop all the edges children.  See if the neighbor already has the node,
...
Par::Out() << std::endl;
  ...
  if (obj.get_type() == MeshObj::ELEMENT) {
    ...
    if (topo->parametric_dim == 3) {
      ...
      // Connect child faces to child elements and share any child nodes of
      // the faces with neighbor elements.
      // One other detail: add FOSTERS'ed edges to the elements at this time,
      // since they naturally are found via the face loop.
      face_resolution(mesh,
                      obj,
                      rtopo,
                      children,
                      nodes);
