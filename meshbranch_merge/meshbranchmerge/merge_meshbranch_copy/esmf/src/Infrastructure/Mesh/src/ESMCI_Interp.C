...
#include "../../Util/include/ESMC_Macros.h"
...
// ASC: this is not used at all in this file... >???
//extern "C" int FTN(f_esmf_getmeshdistgrid)(int*, int*, int*, int*);
...
{}
  ...
  // ASC: are we sure about this "n=0"?
void operator()(MeshObj *obj, UInt nvalset, UInt n=0) {
...
void mat_patch_serial_transfer(MEField<> &src_coord_field, MEField<> &_sfield, _field &_dfield, SearchResult &sres,  Mesh &srcmesh, IWeights &iw) {
    ...
    for (UInt n = 0; n < npts; n++) {
     ...
     //     UInt fdim = dfield->dim();
	    ...
	    double sval = 0;
            ...
            dof_add_col addc(col, dfield->dim(snode), sens);
            
            sF.dof_iterator<dof_add_col>(addc);
            
            iw.InsertRow(row, col);
            
#ifdef CHECK_SENS
            int dof_div_dim = nlocal_dof/dfield->dim(snode);
	    ...
	    if (std::fabs(diff) > 1e-4) {
	      for (UInt s = 0; s < nlocal_dof/dfield->dim(snode); s++) {
 ...
 void mat_point_serial_transfer(MEField<> &sfield, _field &dfield, SearchResult &sres, IWeights &iw) {
  ...
  for (; sb != se; sb++) {
    ...
    std::vector<fad_type> ires(npts*sfield.dim());
    ...
    for (UInt n = 0; n < npts; n++) {
      ...
      for (UInt d = 0; d < 1; d++) {
        ...
        ((double*)dfield.data(node))[d] = ires[n*dfield.dim(node)+d].val();
...
double sval = 0;
        ...
        dof_add_col addc(col, dfield.dim(node), sens);
...
#ifdef CHECK_SENS
        for (UInt s = 0; s < nlocal_dof/dfield.dim(node); s++) {
...
static GeomRend::DstConfig get_dst_config(Mesh &dest, const std::vector<Interp::FieldPair> &fpairs) {
  ...
  Interp::Interp(Mesh &src, Mesh &dest, const std::vector<FieldPair> &_fpairs, bool _is_dg, int unmappedaction) :
sres(),
...
is_dg(_is_dg)
{
  ...
  for (UInt i = 0; i < fpairs.size(); i++) {
    ...
    if (fpairs[i].idata != FORWARD_ONLY) {
...
void Interp::operator()() {
  ...
  if (is_parallel && !is_dg) transfer_parallel(); else transfer_serial();
  
}

void Interp::operator()(UserFunc *_ufunc) {
  Trace __trace("Interp::operator()()");  
...
void Interp::operator()(int fpair_num, IWeights &iw) {
  ...
  if (is_parallel && !is_dg) mat_transfer_parallel(fpair_num, iw); else mat_transfer_serial(fpair_num, iw);
...
void Interp::transfer_serial() {
  ...
  if (fpairs[0].second->is_nodal() && has_std) {
    ...
    point_serial_transfer(srcmesh, ufunc, srcF.size(), &(*srcF.begin()), &(*dstf.begin()), &iflag[0], sres);
