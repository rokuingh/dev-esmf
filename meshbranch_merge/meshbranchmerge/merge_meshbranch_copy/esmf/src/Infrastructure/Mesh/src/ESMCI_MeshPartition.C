...
namespace ESMCI {
  ...
  //ASC this seems valid
  void GetMetisPartition(const Mesh &mesh, UInt npart, const MEField<> &ep) {
    //mesh.Print();
    // Step One is to get the partition array
    int nelem = mesh.num_elems();

    // this is the too many partitions case
    if(npart > nelem){
      // Bombs
      Throw() << "Number of partitions bigger than number of elements: " << npart << " > " << nelem << std::endl;
    }

    // this is the one partition case
    if(npart == 1){
    ...
    // this is the one element per partition case
    if(npart == UInt(nelem)){
      int i = 0;
      MeshDB::const_iterator ei = mesh.elem_begin(), ee = mesh.elem_end();
      for (;ei != ee; ++ei) {
	...
	locadjncy.insert(neighbor->get_data_index());
	//std::cout << "pushing " << neighbor->get_data_index() << ",";
	lcount++;
      }
    ...
    */
    ...
    // Call Metis
    {
      ...
      int npart1 = npart;
      METIS_PartGraphKway(&nelem, &*xadj.begin(), &*adjncy.begin(), NULL, NULL, &wgtflag,
			  &numflag, &npart1, options, &edgecut, &*part.begin());
    }
	...
	if ((UInt) proc==nm) {
	  ...
	  for (UInt s = 0; s < etopo->num_sides; s++) {
	    ...
	    if (si != elem.Relations.end() && GetMeshObjContext(*si->obj).is_set(Attr::EXPOSED_BOUNDARY_ID)) {
	      MeshObj &side = *si->obj;
	      //std::cout << "Found side, side =" << side.get_id() << MeshObjTypeString(mesh.side_type()) << std::endl;
  ...
  void MeshConcat(Mesh &mesh, std::vector<Mesh*> &srcmesh) {
	...
	for (; fi != fe; ++fi) {
	  ...
	  //std::cout << "Creating field:" << ofield.name() << std::endl;
	  MEField<> *efield = mesh.RegisterField(ofield.name(), MEFamilyDG::instance(1), MeshObj::ELEMENT, ctxt, ofield.dim(), true);
    ...
    MEField<> *eown = mesh.RegisterField("_processor", MEFamilyDG::instance(1), MeshObj::ELEMENT,
