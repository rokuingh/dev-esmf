...
typedef enum {update_sf=0x01,    // shape functions
              ...
              update_jinv=0x10   // jacobian inverse
              } update_flag_enum;
};
 ...
 * It also precomputes everything it can at the Kernel level, so,
 * for instance, when blasting through a kernel since all elements
 * will be of the same type and since the intg rule will be the same,
 * shape functions, shape parametric grads, etc may be precomputed
 * and stored once for the whole kernel, rather than at each element.
 * Per element we use the mapping data to then produce Jx, grads, etc...
 * 
 ...
 */
virtual void Setup(const Kernel &ker, UInt uflag, const std::vector<const intgRule*> &ir_list,
       const Mapping<typename ME2MPTraits<METRAITS>::value> *map = NULL);
...
void GetFunctionValues(const FIELD &field, field_type values[]) {
  GetFunctionValues(field, values, field.dim());
}

void GetFunctionValues(const FIELD &field, field_type values[], UInt rstride);

template <typename FTYPE>
void GetFunctionValues(const FIELD &field, const FTYPE mcoef[], FTYPE values[]) {
  GetFunctionValues(field, mcoef, values, field.dim());
}

template <typename FTYPE>
void GetFunctionValues(const FIELD &field, const FTYPE mcoef[], FTYPE values[], UInt rstride) {
  ...
  bmg->meptr->operator()(METraits<FTYPE,double>())->function_values(
        ...
        &mcoef[0], &values[0], &sf[bmg->cur_sf_offset], rstride);

}

/**
 * Gather the coefficients for the master element.  If your element is DG,
 * you should probably skip this and get the data pointer directly to avoid
 * the copy, below.
 */
template <typename Real>
void GetElemData(const FIELD &field, Real values[]) {
  ...
  if (bmg->meptr->orientation() == ShapeFunc::ME_DG) {
    ...
    // Gather step.
    GatherElemData<METRAITS,FIELD, Real, typename METRAITS::field_type>(*bmg->meptr, field, *cur_elem, values);
 ...
 */
void GetFunctionGrads(const FIELD &field, field_type grads[], UInt rstride);

void GetFunctionGrads(const FIELD &field, field_type grads[]) {
  GetFunctionGrads(field, grads, field.dim());
}

template <typename FTYPE>
void GetFunctionGrads(const FIELD &field, const FTYPE mcoef[], FTYPE grads[]) {
  GetFunctionGrads(field, mcoef, grads, field.dim());
}

template <typename FTYPE>
void GetFunctionGrads(const FIELD &field, const FTYPE mcoef[], FTYPE grads[], UInt rstride) {
  ...
  bmg->meptr->operator()(METraits<FTYPE,double>())->function_grads(
                        ...
                        &grads[0], &sfg[bmg->cur_sfg_offset], rstride);
...
const double *GetQPoints(UInt q) { return &bmg->irule->locations()[q*bmg->pdim]; }
...
void GetUnitNormals(double unit_normals[]);

void GetNormals(double normals[]);
