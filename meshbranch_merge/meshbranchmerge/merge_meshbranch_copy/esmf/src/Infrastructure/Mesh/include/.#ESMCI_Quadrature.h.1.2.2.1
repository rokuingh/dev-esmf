...
// Lobatto Legendre on [-1,1]
class barlb : public intgRule {
public:
  static barlb &instance(UInt q);
  ~barlb();
  static const std::string name;
  const std::string &iname() const {return name;}
  const intgRule *ChangeOrder(UInt q) const {
...
private:
  barlb(UInt ord);  // order of quadrature
  static std::map<UInt,barlb*> classInstances;
};
...
// Lobatto Legendre tensor on [-1,1]^2
class quadlb : public intgRule {
public:
  static quadlb &instance(UInt order);
  ~quadlb();
  static const std::string name;
  const std::string &iname() const {return name;}
  const intgRule *ChangeOrder(UInt q) const {
  ...
  const intgRule *side_rule() const {
    return &barlb::instance(q);
  }
private:
  quadlb(UInt ord);  // order of quadrature
  static std::map<UInt,quadlb*> classInstances;
};
