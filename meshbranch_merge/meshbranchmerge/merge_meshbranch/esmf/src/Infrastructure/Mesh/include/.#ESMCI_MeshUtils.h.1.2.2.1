...
template<typename METRAITS=METraits<>, typename FTYPE=MEField<>,
     typename RESTYPE=typename METRAITS::field_type, typename DATATYPE=RESTYPE>
...
/**
 * Same as above, but gather to the side's view, by providing the orientation info
 * about the side to element.
 */
template<typename METRAITS=METraits<>, typename FTYPE=MEField<>, typename RESTYPE=typename METRAITS::field_type>
struct GatherSideData1 {
GatherSideData1(const MasterElement<METRAITS> &me, const FTYPE &f,
        const MeshObj &elem, UInt side_ordinal, bool polarity, UChar rotation, RESTYPE res[]);
};
...
template<typename METRAITS, typename FTYPE>
void ScatterSideData1(const MasterElement<METRAITS> &me, const FTYPE &f,
        const MeshObj &elem, UInt side_ordinal, bool polarity, UChar rotation, const double mcoef[]);
...
/**
 * Calculate an approximate diameter of element.
 */
