                          ...
                          std::vector<MeshObj::id_type> &new_ids)
{
  ...
  if (nproc > 1) {
    if(sizeof(int)==8)MPI_Allgather(&nnew_ids_l, 1, MPI_LONG_LONG, &nnew_ids_all[0], 1, MPI_LONG_LONG, Par::Comm());
  ...
  if (nproc > 1) {
    ...
    if(sizeof(int)==8)MPI_Allreduce(&nused_ids_l, &nused_ids, 1, MPI_LONG_LONG, MPI_SUM, Par::Comm()); 
  } else {
  ...
  { // send to rendezvous
    ...
    //Par::Out() << "SEND PROCS = " <<  send_procs[0] << std::endl;
    ...
    //Par::Out() << "SEND SIZES = " <<  send_sizes[0] << std::endl;
    ...
    for (int i = 0; i < nused_ids_l; i++ ) {
      ...
      SparsePack<MeshObj::id_type>(b, id);
    }
    ...
    for (std::vector<UInt>::iterator p = msg.inProc_begin(); p != msg.inProc_end(); p++) {
      ...
      UInt nids = b.msg_size() / SparsePack<MeshObj::id_type>::size();
  ...
  if (nproc > 1) {
    ...
    if(sizeof(int)==8)MPI_Allgather(&num_avail_l, 1, MPI_LONG_LONG, &num_avail[0], 1, MPI_LONG_LONG, Par::Comm());
  } else {
