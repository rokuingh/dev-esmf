...
T Factorial(T number) {
  ...
  temp = number * Factorial<T>(number - T(1));
  return temp;
}


template<typename T>
T Gamma(const T& number) {
    return Factorial<T>(number-T(1));
}

  // Karni and Sherwin p. 350 1st edition
double Jacobi(double x,double alpha,double beta,int n){
  if(n == 0){
    return double(1.);
  }else if(n == 1){
    return double(.5)*(alpha-beta+(alpha+beta+double(2.))*x);
  }else{
    double a1,a2,a3,a4;
    double nd = double(n),one=double(1.),two=double(2.);
    int i;

    double Pnp1=0,Pn=0,Pnm1=0;

    Pn     = Jacobi(x,alpha,beta,1);
    Pnm1   = Jacobi(x,alpha,beta,0);

    for(i=1;i<n;i++){

      nd = double(i);

      a1 = two*(nd+one)*(nd+alpha+beta+one)*(two*nd+alpha+beta);
      a2 = (two*nd+alpha+beta+one)*(alpha*alpha - beta*beta);
      a3 = (two*nd+alpha+beta)*(two*nd+alpha+beta+one)*(two*nd+alpha+beta+two);
      a4 = two*(nd+alpha)*(nd+beta)*(two*nd+alpha+beta+two);
...
double DerJacobi(double x,double alpha,double beta,int n){
  if(n == 0){
    return double(0.);
  }else if(n == 1){
    return double(1.0);
    ...
    Pn     = Jacobi(x,alpha,beta,n);
    Pnm1   = Jacobi(x,alpha,beta,n-1);

    b1 =(two*nd+alpha+beta)*(one -x*x);
    b2 = nd*(alpha-beta-(two*nd+alpha+beta)*x);
    b3 = two*(nd+alpha)*(nd+beta);

    return (b2*Pn + b3*Pnm1)/b1;
  }
}



static void JacobiRootFinder(double x[],int n,double alpha,double beta){

  double *xcheby = new double[n];
  double r,s,delta=10000.0; 
  double one=double(1.),zero=double(0.);
  double Pm,DerPm;

  int i,k;

  // This is the guess to the root
  for(k=0;k<n;k++)xcheby[k] = -std::cos(M_PI*(double(2*k)+one)/double(2*n));

  for(k=0;k<n/2;k++){
    delta =100000.0;
    r = xcheby[k];

    if(k>0)r=(r+x[k-1])*double(.5);

    while(delta > 2.0E-16){
      s = zero;
      for(i=0;i<k;i++)s=s+one/(r-x[i]);
      Pm = Jacobi(r,alpha,beta,n);
      DerPm = DerJacobi(r,alpha,beta,n);
      delta = -Pm/(DerPm - Pm*s);
      r=r+delta;
    }
    x[k]=r;
...
static double C(double x[],int i,int Q,double alpha,double beta){

  double h=double(1.0);
  double one=double(1.),two=double(2.);

  h=h*std::pow(two,alpha+beta+one);
  h=h*Gamma<double>(alpha+double(Q));
  h=h*Gamma<double>(beta +double(Q));
  h=h/(double(Q)-one);
  h=h/Factorial<double>((double(Q)-one));
  h=h/Gamma<double>(alpha+beta+double(Q)+one);
  h=h/std::pow(Jacobi(x[i],alpha,beta,Q-1),two);

  return h;
};

void lobatto_legendre_set(int N,double _LGLP[],double _LGLW[]){
  JacobiRootFinder(&_LGLP[1],N-2,1.,1.);
  _LGLP[0]=double(-1.);
  _LGLP[N-1]=double(1.);
  int k;
  for(k=0;k<N;k++)_LGLW[k] = C(_LGLP,k,N,0.0,0.0);
};
...
// ** 1D bar, lobatto
std::map<UInt,barlb*> barlb::classInstances;

const std::string barlb::name("barlb");

barlb &barlb::instance(UInt q) {
  std::map<UInt,barlb*>::iterator qi = classInstances.find(q);
  barlb *qp;
  if (qi == classInstances.end()) {
    qp = new barlb(q);
    classInstances[q] = qp;
...
intgRule(q, q, 1)
{
  lobatto_legendre_set(n, locs, wgts);
}
...
// ** 2D quadrilateral lobatto
std::map<UInt,quadlb*> quadlb::classInstances;

const std::string quadlb::name("quadlb");

quadlb &quadlb::instance(UInt q) {
  std::map<UInt,quadlb*>::iterator qi = classInstances.find(q);
  quadlb *qp;
  if (qi == classInstances.end()) {
    qp = new quadlb(q);
    classInstances[q] = qp;
...
quadlb::quadlb(UInt tq) :
intgRule(tq, tq*tq, 2)
{
  std::vector<double> tlocs(q), twgts(q);
  lobatto_legendre_set(q, &tlocs[0], &twgts[0]);
...
intgRule *Topo2Intg::operator()(UInt q, const std::string &name) {
  ...
  if (name == "QUAD9") {
      return &quadq::instance(q);
  } else if (name == "TRI" || name == "TRI3" || name == "TRI3_3D" || name == "TRISHELL") {
