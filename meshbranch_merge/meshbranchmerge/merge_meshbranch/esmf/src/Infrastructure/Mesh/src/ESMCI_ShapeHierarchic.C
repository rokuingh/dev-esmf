...
Par::Out() << name() << " interpMatrix:" << std::endl;
    for (UInt f = 0; f < nfunc; f++) {
...
template<typename Real>
void bh_shape_elem(UInt pdim, UInt ndofs, const std::vector<ILegendre<Real> > &func,
              UInt npts, const Real pcoord[], Real results[]) {
...
void bh_shape(UInt pdim, UInt ndofs, const std::vector<ILegendre<Real> > &func,
  ...
  bh_shape_elem(pdim, ndofs, func, npts, pcoord, &results[2]);
...
{
  Throw() << "No edge on Quad hier, please dont call";
}
...
{
  bh_shape_elem(ParametricDim(), stride, ild, npts, pcoord, res);
}
 ...
 ShapeHier(_q)
{
  ...
  // Edge perm table (no perm)
  m_edge_perm.resize(2*(q-1));
  for (UInt i = 0; i < (q-1); i++) {
    m_edge_perm[i] = m_edge_perm[q-1+i] = i;
  }
 ...
 ShapeHier(_q)
{
  ...
  // TODO: figure out face perm for this guy (difficult).
