...
{
  Trace __trace("GetObjList(void *user, int numGlobalIds, int numLids, ZOLTAN_ID_PTR gids, ZOLTAN_ID_PTR lids, int wgt_dim, float *obj_wghts, int *err)");
  GeomRend::ZoltanUD &udata = *(static_cast<GeomRend::ZoltanUD*>(user));
...
void GeomRend::build_src_mig(Zoltan_Struct *zz, ZoltanUD &zud) {
  ...
  ESMCI::rcb_isect(zz, coord, zud.srcObj, mignode, dcfg.geom_tol, sdim);
  ...
  if (dcfg.neighbors)
    ESMCI::add_neighbors(src_migration);
     ...
     ZOLTAN_ID_PTR exportLids, ZOLTAN_ID_PTR exportGids, int *exportProcs)
{
  ...
  if (iter_is_obj) {
    ...
    ESMCI::rcb_isect(zz, coord, zud.dstObj, mignode, dcfg.geom_tol, sdim);
...
void GeomRend::prep_meshes() {
  ...
  if (iter_is_obj) {
    ...
    std::vector<_fieldLocale>::const_iterator locb = cf->locale_begin(), loce = cf->locale_end();
 
    // Register the field
    for (; locb != loce; ++locb) {
      dstmesh_rend.Registerfield(cf->name(),
                *locb, dcoord.FType());
    }
...
void GeomRend::migrate_meshes() { 
  ...
  if (iter_is_obj) {
    ...
    _field *dc_rf = dstmesh_rend.Getfield("_me_coordinates");
    ThrowRequire(dc_rf);
...
void GeomRend::Build(UInt nsrcF, MEField<> **srcF, UInt ndstF, MEField<> **dstF) {
  ...
  Zoltan_Set_Num_Obj_Fn(zz, ESMCI::GetNumAssignedObj, (void*) &zud);
  Zoltan_Set_Obj_List_Fn(zz, ESMCI::GetObjList, (void*) &zud);
  Zoltan_Set_Num_Geom_Fn(zz, ESMCI::GetNumGeom, (void*) &zud);
  Zoltan_Set_Geom_Multi_Fn(zz, ESMCI::GetObject, (void*) &zud);
    ...
    if (iter_is_obj) {
      if (fptr) {
        dst_rend_Fields.push_back(dstmesh_rend.RegisterField(fptr->name(), fptr->GetMEFamily(),
                   fptr->ObjType(), fptr->GetContext(), fptr->dim(), true, false, fptr->FType()));
      } else dst_rend_Fields.push_back(0);
      ...
      if (fptr) {
        ...
        std::vector<_fieldLocale>::const_iterator locb = ifptr->locale_begin(), loce = ifptr->locale_end();
   
        // Register the field
        _field *nf;
        for (; locb != loce; ++locb) {
  
          nf = dstmesh_rend.Registerfield(ifptr->name(),
                    *locb, fptr->FType());
        }
  
        dst_rend_fields.push_back(nf);
      } else dst_rend_fields.push_back(0);
